...
const HotelDetail = () => {
  ...
  if (!hotel) {
    return (
      <div className="pt-24 pb-20 min-h-screen flex flex-col items-center justify-center">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-4">Hotel not found</h1>
  ...
  return (
    <div className="pt-20 pb-20 min-h-screen bg-gray-50 dark:bg-[#0f0f1a]">
      <SEO
        title={`${hotel.name} — Book Hotel | HimQueenKing`}
        description={`${hotel.name} in ${hotel.location}. ${hotel.desc}`}
        keywords={`${hotel.name}, ${hotel.location} hotel, himachal hotel, ${hotel.location} accommodation`}
        url={`https://himqueenking.onrender.com/hotels/${hotel.id}`}
      />
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <Link to="/hotels" className="inline-flex items-center gap-2 text-gray-600 dark:text-gray-400 hover:text-[#b8860b] font-medium mb-6 mt-4 transition">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white dark:bg-[#1a1a2e] rounded-2xl p-6 shadow-sm">
              <div className="flex items-center gap-3 mb-4">
                <div className="flex items-center gap-1 bg-amber-50 dark:bg-amber-900/30 px-3 py-1.5 rounded-full">
                  ...
                  <span className="text-xs text-gray-500 dark:text-gray-400">({hotel.reviews} reviews)</span>
                </div>
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-6">{hotel.desc}</p>

              <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-3">Amenities</h2>
              <div className="flex flex-wrap gap-2 mb-6">
                {hotel.amenities.map((a) => (
                  <span key={a} className="px-3 py-1.5 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 text-sm font-medium">{a}</span>
                ))}
              </div>

              <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-3">Features</h2>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 space-y-6">
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="bg-white dark:bg-[#1a1a2e] rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700">
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">Get a custom quote based on your dates and room preferences</p>
              </motion.div>
