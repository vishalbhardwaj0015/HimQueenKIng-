  ...
  if (!tour) {
    return (
      <div className="pt-24 pb-20 min-h-screen flex flex-col items-center justify-center">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-4">Tour not found</h1>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white dark:bg-[#1a1a2e] rounded-2xl p-6 shadow-sm">
              <div className="flex flex-wrap gap-3">
                <div className="flex items-center gap-2 text-gray-700 dark:text-gray-300 text-sm bg-gray-50 dark:bg-[#0f0f1a] px-4 py-2 rounded-full font-medium">
                ...
                <div className="flex items-center gap-1 text-sm bg-amber-50 dark:bg-amber-900/30 px-4 py-2 rounded-full font-medium">
            ...
            {tour.route && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="bg-white dark:bg-[#1a1a2e] rounded-2xl p-6 shadow-sm">
                <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-4 flex items-center gap-2">
                  <Route size={20} className="text-[#b8860b]" /> Route
                </h2>
                <div className="bg-[#faf9f6] dark:bg-[#0f0f1a] rounded-xl p-4">
                  <p className="text-sm sm:text-base text-gray-800 font-semibold">{tour.route}</p>
                </div>
              </motion.div>
            )}

            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white dark:bg-[#1a1a2e] rounded-2xl p-6 shadow-sm">
              <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-4">Main Highlights</h2>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="bg-white dark:bg-[#1a1a2e] rounded-2xl p-6 shadow-sm">
              <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-6">Day-by-Day Itinerary</h2>
                ...
                {tour.itinerary.map((day, i) => (
                  <div key={i} className="flex gap-4">
                    ...
                    <div className="pb-5 flex-1">
                      <div className="bg-gray-50 dark:bg-[#0f0f1a] rounded-xl p-4">
                        <h3 className="font-bold text-gray-900 dark:text-gray-100 text-sm sm:text-base">{day.title}</h3>
                        ...
                        {day.meals && (
                          <p className="mt-2 text-xs text-green-700 dark:text-green-400 bg-green-50 dark:bg-green-900/30 inline-block px-2 py-1 rounded-full font-medium">🍽 {day.meals}</p>
                        )}
                      </div>
                    </div>
                  </div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-white dark:bg-[#1a1a2e] rounded-2xl p-6 shadow-sm">
              <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-4">Package Includes</h2>
              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {tour.includes.map((inc) => (
                  <li key={inc} className="flex items-start gap-2 text-sm text-gray-700 dark:text-gray-300">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="bg-white dark:bg-[#1a1a2e] rounded-2xl p-6 shadow-sm">
              <h2 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-4">Package Excludes</h2>
              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {tour.excludes.map((exc) => (
                  <li key={exc} className="flex items-start gap-2 text-sm text-gray-700 dark:text-gray-300">
                    <XCircle size={15} className="text-red-400 dark:text-red-400 mt-0.5 shrink-0" /> {exc}
                  </li>
                ))}
              </ul>
            </motion.div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 space-y-6">
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="bg-white dark:bg-[#1a1a2e] rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700">
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">Get a custom quote based on your exact dates and preferences</p>
              </motion.div>
