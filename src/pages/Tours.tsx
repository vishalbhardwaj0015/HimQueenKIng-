...
const Tours = () => {
  ...
  const diffColor: Record<string, string> = { Easy: "text-green-600 bg-green-50", Moderate: "text-amber-600 bg-amber-50", Challenging: "text-orange-600 bg-orange-50", Hard: "text-red-600 bg-red-50" };
    ...
    <div className="pt-20 pb-20 min-h-screen bg-[#faf9f6]">
      <div className="relative h-64 sm:h-80 bg-[#1a1a2e] overflow-hidden">
        ...
        <div className="relative z-10 h-full flex flex-col items-center justify-center text-center px-4">
          ...
          <p className="text-gray-300 max-w-lg">Handcrafted all-inclusive tours through the majestic Himalayas</p>
        </div>
      </div>

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8 relative z-20">
        <div className="bg-white rounded-2xl shadow-lg p-4 flex items-center gap-3 mb-8 border border-gray-100">
            ...
            <motion.article key={tour.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }} className="card group">
              <Link to={`/tours/${tour.id}`}>
                <div className="relative h-56 overflow-hidden">
                  ...
                  <span className={`absolute bottom-3 right-3 px-2.5 py-1 rounded-full text-xs font-semibold text-white ${budgetRanges[tour.budget].color}`}>
                    {budgetRanges[tour.budget].label}
                  </span>
              ...
              <div className="p-6">
                <h3 className="text-lg font-bold text-gray-900 mb-2 group-hover:text-[#b8860b] transition-colors">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                  <div className="text-xs text-gray-400">{tour.duration} · {tour.difficulty}</div>
        ...
        {filtered.length === 0 && (
          <div className="text-center py-20 text-gray-400">
            ...
            <p className="text-sm">Try adjusting your budget or difficulty filters</p>
          </div>
        )}
      </div>
    </div>
